...
export default class Mover {
  ...
  startMoving(card) {
    ...
    function handleMove(event) {
      ...
      event.currentTarget.after(card.htmlClone);
      const moveHereButtons = document.getElementsByClassName("moveHere");
      console.log("moveHereButtons !!!!!!!!!!!!!!!!");
      console.log(moveHereButtons);
    ...
    if (this.canMove) {
      for(let col = 0; col < colTitles.length; col++) {
          ...
          clone.addEventListener("click", handleMove.bind(this));
      ...
      for(let i = 0; i < cards.length - 1; i++) { // -1 bc this includes the template 
        let clone = moveButton.cloneNode(true);
        clone.addEventListener("click", handleMove.bind(this));  // ! .bind(this) maybe so that this refers to mover obj??? also it says to bind in constructor but i didnt do that...
    ...
    this.canMove = false;  // ! reset to true when we end move
  }
